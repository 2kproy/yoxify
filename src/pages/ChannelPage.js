...
//   console.log(userId, username, channelId);
  ...
  useEffect(() => {
    ...
    return () => {
      // Когда компонент размонтируется, убираем слушатели
      socket.off('chatMessage');
    };
  }, [channelId]);
  ...
  const handleSend = () => {
    if (!text.trim()) return;
    socket.emit('chatMessage', { channelId, text });
    setText('');
  };
  // return (
  //   <div>
  ...
  //           <em style={{ color: 'gray', marginLeft: '5px' }}>
  //             {new Date(m.createdAt).toLocaleTimeString()}
  //           </em>
  //         </div>
  //       ))}
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ border: '1px solid #ccc', height: 500, overflowY: 'auto', p: 1, mb: 1 }}>
        {messages.map((m, i) => (
